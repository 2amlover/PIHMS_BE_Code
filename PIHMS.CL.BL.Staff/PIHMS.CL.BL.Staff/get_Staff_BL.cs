...
{
class get_Staff_BL : cls_BC_BusinessLogic
{
    public get_Staff_BL(ref cls_Util_DBAccess dbc)
    {
        m_dbc = dbc;
    }

    public int get_Staff(ref get_Staff_IP ipdc, ref get_Staff_OP opdc)
    {
        m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

        List<tbl_mas_staff> ml_Staff = new List<tbl_mas_staff>();
