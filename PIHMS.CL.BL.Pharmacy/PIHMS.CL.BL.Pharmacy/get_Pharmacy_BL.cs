...
{
    class get_Pharmacy_BL : cls_BC_BusinessLogic
    {
        public get_Pharmacy_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_Pharmacy(ref get_Pharmacy_IP ipdc, ref get_Pharmacy_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_trn_pharmacy> ml_Pharmacy = new List<tbl_trn_pharmacy>();
