...
{
    class get_InvStores_BL : cls_BC_BusinessLogic
    {
        public get_InvStores_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_InvStores(ref get_InvStores_IP ipdc, ref get_InvStores_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_inv_stores> ml_Inventory = new List<tbl_mas_inv_stores>();
