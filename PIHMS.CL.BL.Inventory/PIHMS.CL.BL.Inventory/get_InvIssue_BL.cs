...
{
    class get_InvIssue_BL : cls_BC_BusinessLogic
    {
        public get_InvIssue_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_InvIssue(ref get_InvIssue_IP ipdc, ref get_InvIssue_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List < tbl_trn_inv_issue >ml_Inventory = new List<tbl_trn_inv_issue>();
