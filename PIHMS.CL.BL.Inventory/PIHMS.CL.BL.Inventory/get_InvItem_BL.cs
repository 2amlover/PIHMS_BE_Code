...
{
    class get_InvItem_BL : cls_BC_BusinessLogic
    {
        public get_InvItem_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_InvItem(ref get_InvItem_IP ipdc, ref get_InvItem_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List < tbl_mas_inv_item >ml_Inventory = new List<tbl_mas_inv_item>();
