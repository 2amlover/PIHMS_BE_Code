...
{
    class get_MasWard_BL : cls_BC_BusinessLogic
    {
        public get_MasWard_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_MasWard(ref get_MasWard_IP ipdc, ref get_MasWard_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_ward> ml_Masters = new List<tbl_mas_ward>();
