...
{
    class get_MasBranch_BL : cls_BC_BusinessLogic
    {
        public get_MasBranch_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_MasBranch(ref get_MasBranch_IP ipdc, ref get_MasBranch_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_branch> ml_Masters = new List<tbl_mas_branch>();
