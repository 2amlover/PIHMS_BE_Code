...
{
    class get_MasScan_BL : cls_BC_BusinessLogic
    {
        public get_MasScan_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_MasScan(ref get_MasScan_IP ipdc, ref get_MasScan_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_scan> ml_Masters = new List<tbl_mas_scan>();
