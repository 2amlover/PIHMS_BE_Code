...
{
    class put_MasScanTemplate_BL : cls_BC_BusinessLogic
    {
        public put_MasScanTemplate_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }
        public int put_MasScanTemplate(ref put_MasScanTemplate_IP ipdc, ref put_MasScanTemplate_OP opdc)
        {
            if (ipdc.m_Masters.ID > 0)
            {
                //Update Existing Row

                m_rc = put_MasScanTemplate_Update(ref ipdc, ref opdc);
            ...
            else
            {
                ...
                m_rc = put_MasScanTemplate_Insert(ref ipdc, ref opdc);
                Console.WriteLine(" NEW ROW INSERT SUCCESSFULLY, " + ipdc.m_Masters.ID.ToString() + " tbl_mas_branch");
            }
            return m_rc;
        }
        private int put_MasScanTemplate_Update(ref put_MasScanTemplate_IP ipdc, ref put_MasScanTemplate_OP opdc)
        {
            m_WhereCondition = " WHERE ID = " + ipdc.m_Masters.ID.ToString();
            List<tbl_mas_scan_template> m_MastersTemp = new List<tbl_mas_scan_template>();
        ...
        private int put_MasScanTemplate_Insert(ref put_MasScanTemplate_IP ipdc, ref put_MasScanTemplate_OP opdc)
        {
            List<tbl_mas_scan_template> m_MastersTemp = new List<tbl_mas_scan_template>();
            ipdc.m_Masters.CreatedDateTime = DateTime.Now;
            ipdc.m_Masters.UpdatedDateTime = DateTime.Now;
