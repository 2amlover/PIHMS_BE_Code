...
{
    class get_MasAccount_BL : cls_BC_BusinessLogic
    {
        public get_MasAccount_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_MasAccount(ref get_MasAccount_IP ipdc, ref get_MasAccount_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_acc> ml_Masters = new List<tbl_mas_acc>();
