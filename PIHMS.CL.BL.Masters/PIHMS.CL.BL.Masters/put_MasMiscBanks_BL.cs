...
{
    class put_MasMiscBanks_BL : cls_BC_BusinessLogic
    {
        public put_MasMiscBanks_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }
        public int put_MasMiscBanks(ref put_MasMiscBanks_IP ipdc, ref put_MasMiscBanks_OP opdc)
        {
            if (ipdc.m_Masters.ID > 0)
            {
                //Update Existing Row

                m_rc = put_MasMiscBanks_Update(ref ipdc, ref opdc);
            ...
            else
            {
                ...
                m_rc = put_MasMiscBanks_Insert(ref ipdc, ref opdc);
                Console.WriteLine(" NEW ROW INSERT SUCCESSFULLY, " + ipdc.m_Masters.ID.ToString() + " tbl_mas_misc_banks");
            }
            return m_rc;
        }
        private int put_MasMiscBanks_Update(ref put_MasMiscBanks_IP ipdc, ref put_MasMiscBanks_OP opdc)
        {
            m_WhereCondition = " WHERE ID = " + ipdc.m_Masters.ID.ToString();
            List<tbl_mas_misc_banks> m_MastersTemp = new List<tbl_mas_misc_banks>();
        ...
        private int put_MasMiscBanks_Insert(ref put_MasMiscBanks_IP ipdc, ref put_MasMiscBanks_OP opdc)
        {
            List<tbl_mas_misc_banks> m_MastersTemp = new List<tbl_mas_misc_banks>();
            ipdc.m_Masters.CreatedDateTime = DateTime.Now;
            ipdc.m_Masters.UpdatedDateTime = DateTime.Now;
