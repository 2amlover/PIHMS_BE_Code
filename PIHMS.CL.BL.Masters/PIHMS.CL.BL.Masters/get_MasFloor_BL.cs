...
{
    class get_MasFloor_BL : cls_BC_BusinessLogic
    {
        public get_MasFloor_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int get_MasFloor(ref get_MasFloor_IP ipdc, ref get_MasFloor_OP opdc)
        {
            m_WhereCondition = " WHERE ID > 0 order by ID desc limit " + ipdc.m_Filter.m_Limit;

            List<tbl_mas_floor> ml_Masters = new List<tbl_mas_floor>();
