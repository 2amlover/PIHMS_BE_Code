...
{
    class put_PharmacyOpconsultItem_BL : cls_BC_BusinessLogic
    {
        public put_PharmacyOpconsultItem_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }
        public int put_PharmacyOpconsultItem(ref put_PharmacyOpconsultItem_IP ipdc, ref put_PharmacyOpconsultItem_OP opdc)
        {
            if (ipdc.m_DoctorConsultation.ID > 0)
            {
                //Update Existing Row

                m_rc = put_PharmacyOpconsultItem_Update(ref ipdc, ref opdc);
            ...
            else
            {
                ...
                m_rc = put_PharmacyOpconsultItem_Insert(ref ipdc, ref opdc);
                Console.WriteLine(" NEW ROW INSERT SUCCESSFULLY, " + ipdc.m_DoctorConsultation.ID.ToString() + " tbl_trn_op_consult_appointment");
            }
            return m_rc;
        }
        private int put_PharmacyOpconsultItem_Update(ref put_PharmacyOpconsultItem_IP ipdc, ref put_PharmacyOpconsultItem_OP opdc)
        {
            m_WhereCondition = " WHERE ID = " + ipdc.m_DoctorConsultation.ID.ToString();
            List<tbl_trn_pharmacy_opconsult_item> m_DoctorConsultationTemp = new List<tbl_trn_pharmacy_opconsult_item>();
        ...
        private int put_PharmacyOpconsultItem_Insert(ref put_PharmacyOpconsultItem_IP ipdc, ref put_PharmacyOpconsultItem_OP opdc)
        {
            List<tbl_trn_pharmacy_opconsult_item> m_DoctorConsultationTemp = new List<tbl_trn_pharmacy_opconsult_item>();
            ipdc.m_DoctorConsultation.CreatedDateTime = DateTime.Now;
            ipdc.m_DoctorConsultation.UpdatedDateTime = DateTime.Now;
