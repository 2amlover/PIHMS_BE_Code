...
{
    class put_PatientHistory_BL : cls_BC_BusinessLogic
    {
        public put_PatientHistory_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }

        public int put_PatientHistory(ref put_PatientHistory_IP ipdc, ref put_PatientHistory_OP opdc)
        {
            if (ipdc.m_Patient.ID > 0)
            {
                //Update Existing Row

                m_rc = put_PatientHistory_Update(ref ipdc, ref opdc);
            ...
            else
            {
                ...
                m_rc = put_PatientHistory_Insert(ref ipdc, ref opdc);
                Console.WriteLine(ipdc.m_Patient.ID.ToString() + " NEW ROW INSERT SUCCESSFULLY, ");
            }
            return m_rc;
        }
        private int put_PatientHistory_Update(ref put_PatientHistory_IP ipdc, ref put_PatientHistory_OP opdc)
        {
            m_WhereCondition = " WHERE ID = " + ipdc.m_Patient.ID.ToString();
            List<tbl_trn_patient_history> m_PatientTemp = new List<tbl_trn_patient_history>();
        ...
        private int put_PatientHistory_Insert(ref put_PatientHistory_IP ipdc, ref put_PatientHistory_OP opdc)
        {
            List<tbl_trn_patient_history> m_PatientTemp = new List<tbl_trn_patient_history>();
            ipdc.m_Patient.CreatedDateTime = DateTime.Now;
            ipdc.m_Patient.UpdatedDateTime = DateTime.Now;
