...
{
    class get_IPCaseOTConsumables_BL : cls_BC_BusinessLogic
    {
        public get_IPCaseOTConsumables_BL(ref cls_Util_DBAccess dbc)
        {
            m_dbc = dbc;
        }
        public int get_IPCaseOTConsumables(ref get_IPCaseOTConsumables_IP ip, ref get_IPCaseOTConsumables_OP op)
        {
            ...
            List<tbl_trn_ipcase_consumeable_detail> ml_IPCaseConsumeableDetailTemp = new List<tbl_trn_ipcase_consumeable_detail>();

            m_rc = m_dbc.Select(ref m_ErrorMessage, m_WhereCondition, ref ml_IPCaseConsumeableDetailTemp);
            if (m_rc < 0)
            {
                op.ReturnMessage = m_ErrorMessage;
                op.ReturnValue = m_rc;
            ...
            op.ml_IPCaseConsumeableDetail = ml_IPCaseConsumeableDetailTemp;
            return m_rc;
        }
